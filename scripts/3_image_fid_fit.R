...
# This script determines the pixel coordinates of the principal point for each image
...
# combination of distances with the smallerst RMSE. Then, georeference the film
# coordinates to the image coordinatess, and translate the first fiducial to
...
# Once the fids are translated, find the rotatio angle that minimizes the RMSE
# among the corresponding film and image measurements (rotate the coordinates
# about Fid 1):
otimized_rotation <- optimize(rot, c(-2 * pi, 2 * pi), best_image_fids)
rms.xy <- otimized_rotation$objective  #report this value for planimetric error (units of pixels...should be < 1)
print(rms.xy) # Fit among pixels
x <- otimized_rotation$minimum # Extract th etatget angle
# Rotate the data:
...
# Calcualte error in the fit among image pixel centers and those predicted by the calibration measurements (should be sub-pixel:
...
image_1 <- list(image_no[1],fid_cal,ppa2cpp,mission_res,focal_cal_px,rms_fids,rms_imagefids) # Gather the results
names(image_1) <- c("Images","Image coordinates of the fiducial marks (pixels)","Calibrated principal point","Image resolution (mm)","focal length (pixels)","RMS of the fids","Fiducial placement RMS")
print(image_1)
